...
class Deformation(nn.Module):
    def __init__(self, D=8, W=256, input_ch=27, input_ch_time=9, grid_pe=0, skips=[], args=None):
        ...
        self.grid = HexPlaneField(args.bounds, self.kplanes_config, args.multires)
        # breakpoint()
        self.args = args
    ...
    def query_time(self, rays_pts_emb, scales_emb, rotations_emb, time_feature, time_emb):
        ...
        if self.no_grid:
            ...
            grid_feature = self.grid(rays_pts_emb[:,:3], time_emb[:,:1]) #this is the place that calls the hexplane
            # breakpoint()
            if self.grid_pe > 1:
...
class deform_network(nn.Module):
    def __init__(self, args) :
        ...
        self.hexplane_params = self.get_grid_parameters()
        self.mlp_parameters = self.get_mlp_parameters()
        number_of_hexplane_params = sum([i.numel() for i in self.hexplane_params])
        number_of_mlp_params = sum([i.numel() for i in self.mlp_parameters])
        total_params = number_of_hexplane_params + number_of_mlp_params
        print(f"total number of params is {total_params}")
        scaled_neural_ode_lr = 0.00016
        scaled_encoder_lr = 0.0016
        self.hexplane_optimizer = torch.optim.Adam(self.hexplane_params, lr=scaled_encoder_lr)
        self.neural_ode_optimizer = torch.optim.Adam(self.mlp_parameters, lr=scaled_neural_ode_lr) #just calling this neural_ode so that i wont have to change 
        ...
        self.schedulers = { 
            "encoder_scheduler":torch.optim.lr_scheduler.ExponentialLR(
                self.hexplane_optimizer, gamma=0.1 ** (1.0 / args.dynamic_max_steps)
            ), 

            "neural_ode_scheduler": torch.optim.lr_scheduler.ExponentialLR(
                self.neural_ode_optimizer, gamma=0.1 ** (1.0 / args.dynamic_max_steps)
            ), 
        }

        # print(self)

    def forward(self, initial_params, times_sel=None):
        """
        Assume the order is always means, squats, quats from the initial_params
        Time is stored as [t0, t1]
        Here we are just brute forcing stuff.
        
        Also return the parameters against the initial parameters
        """
        point = initial_params[..., :3]
        quats = initial_params[..., 3:7]
        scales = initial_params[..., 7:10]
        times = times_sel[1].repeat(point.shape[0],1).to("cuda").to(torch.float32)
        means_pred, scales_pred, rotations_pred = self.forward_dynamic(point=point, rotations=quats, scales=scales,  times_sel=times)
        
        pred_params = torch.cat((means_pred, rotations_pred, scales_pred), dim=-1)
        result = torch.stack((initial_params, pred_params), dim=0)
        return result
