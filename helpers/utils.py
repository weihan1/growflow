...
def setup_directories(result_dir):
    paths = {
        "ckpt_dir": f"{result_dir}/ckpts",
        "stats_dir_static": f"{result_dir}/stats/static",
...
def verify_optimizer_parameter_references(gaussians):
    ...
    for name, optimizer in gaussians.optimizers.items():
        ...
        if id(param_in_optimizer) == id(actual_param):
            ...
            print(f"✗ Mismatch for parameter '{name}'")
            print(f"  Optimizer tracking: {id(param_in_optimizer)}")
...
def get_divisors(n):
    ...
    if n <= 0:
        return []
    divs = set()
    # Iterate from 1 up to and including the integer square root of n
    for i in range(1, math.isqrt(n) + 1):
        if n % i == 0:
            divs.add(i)
            # If i and n//i are different, add the complementary divisor
            if i * i != n:
                divs.add(n // i)
