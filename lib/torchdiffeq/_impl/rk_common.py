...
def _runge_kutta_step(func, y0, f0, t0, dt, t1, tableau):
    ...
    k = _UncheckedAssign.apply(k, f0, (..., 0)) #assigning k[...,0] = f0, computing k1
    for i, (alpha_i, beta_i) in enumerate(zip(tableau.alpha, tableau.beta)): #this loops run 6 times
        if alpha_i == 1.:
            # Always step to perturbing just before the end time, in case of discontinuities.
            ti = t1
            perturb = Perturb.PREV
        else:
            ti = t0 + alpha_i * dt #This is t0 + c2h in the first iteration
            perturb = Perturb.NONE
        yi = y0 + torch.sum(k[..., :i + 1] * (beta_i * dt), dim=-1).view_as(f0) #y_0 + ha_21k_1 in the first iteration, this part is essentially the integral.
        f = func(ti, yi, perturb=perturb) 
        k = _UncheckedAssign.apply(k, f, (..., i + 1)) #assigning k[..., i+1] = f_i

    if not (tableau.c_sol[-1] == 0 and (tableau.c_sol[:-1] == tableau.beta[-1]).all()):
        # This property (true for Dormand-Prince) lets us save a few FLOPs.
        yi = y0 + torch.sum(k * (dt * tableau.c_sol), dim=-1).view_as(f0) #after computing all ki's do final linear combination
    #total of 7 rectangles.
    y1 = yi
    f1 = k[..., -1]
...
class RKAdaptiveStepsizeODESolver(AdaptiveStepsizeEventODESolver):
    ...
    def _before_integrate(self, t):
        ...
        if self.first_step is None:
            first_step = _select_initial_step(self.func, t[0], self.y0, self.order - 1, self.rtol, self.atol,
                                              self.norm, f0=f0) #chooses dt
        else:
            first_step = self.first_step
        self.rk_state = _RungeKuttaState(self.y0, f0, t[0], t[0], first_step, [self.y0] * 5)  #creates a rungekuttastate with new dt
    ...
    def _adaptive_step(self, rk_state):
        ...
        #                      Assertions                      #
        ########################################################
        #NOTE: makes more sense to check for y0 finiteness then checking underflowing
        #NOTE: undeflowing can be caused by stiffness or other y0 being NaN
        ...
        if accept_step:
            ...
            f_next = f1
        else:  #if step is rejected, fallback to previous configurations
