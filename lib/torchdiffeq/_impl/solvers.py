...
class FixedGridODESolver(metaclass=abc.ABCMeta):
    ...
    def integrate(self, t):
        time_grid = self.grid_constructor(self.func, self.y0, t) #returns time here
        ...
        for t0, t1 in zip(time_grid[:-1], time_grid[1:]):
            dt = t1 - t0 #this is h essentially
            ...
            while j < len(t) and t1 >= t[j]:
                if self.interp == "linear":
                    solution[j] = self._linear_interp(t0, t1, y0, y1, t[j]) #the interpolation here will yield y1 if the time matches
