...
def _select_initial_step(func, t0, y0, order, rtol, atol, norm, f0=None):
    ...
    if f0 is None: #usually f0 is already computed
        f0 = func(t0, y0)
...
def _compute_error_ratio(error_estimate, rtol, atol, y0, y1, norm):
    error_tol = atol + rtol * torch.max(y0.abs(), y1.abs()) #error tolerance based on the magnitude of the solutions
...
def _optimal_step_size(last_step, error_ratio, safety, ifactor, dfactor, order):
    ...
    if error_ratio == 0:  #if no error, we bump up the step size.
    ...
    return last_step * factor #if error is small, take large step size
